...
class Forecast extends React.Component {
    ...
    componentWillMount() {
        fetchForecast(this.props.id).then(response => {
            ...
            h1 className = "Forecast-title" > { location } < /h1> {
                weather.map((_, i) => ( <
                    ...
                    li className = "Forecast-day_weather" > {
                        i == 0 ?
                        'Today' :
                            i == 1 ? 'Tomorrow' : weather[i].applicable_date
                    } <
                    /li> <
                    li className = "Forecast-day_weather" >
                    <
                    ...
                    <
                    /li> <
                    li className = "Forecast-day_weather" > { weather[i].weather_state_name } <
                    /li> <
                    li className = "Forecast-day_weather" >
                    Max: { Math.round(weather[i].max_temp) }°
                    C <
                    /li> <
                    li className = "Forecast-day_weather" >
                    Min: { Math.round(weather[i].min_temp) }°
                    C <
                    /li> <
                    li className = "Forecast-day_weather" >
